...
- **Spot positions (`pos_x`, `pos_y`) are NOT taken from the JSON `coords` field.** Instead, the frontend dynamically runs a client-side TypeScript generation module (`frontend/lib/seed.ts`) that executes a **stratified-quadrant Poisson-disc algorithm** (min separation ≥ 16 units, 4-unit margin from all edges) instantly inside the browser. Two seed points are placed in each of the 4 quadrants of the playfield first (securing 8 spots around the perimeter), then Bridson expansion fills the remaining spots radially outward up to 2.5x the minimum radius. This **guarantees** spots appear heavily in all corners and the full extent of the screen.
